...
COVERAGE_REQUIREMENT = os.environ.get("COVERAGE_REQUIREMENT") or "coverage==7.13.5"
...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=src/saltext/bmc/*",
            "--include=tests/*",
        )
        try:
            session.run("coverage", "report", "--show-missing", "--include=src/saltext/bmc/*")
...
def lint_tests(session):
    ...
    """
    flags = [
        "--disable=I,redefined-outer-name,no-member,missing-module-docstring,missing-function-docstring,missing-class-docstring,attribute-defined-outside-init,inconsistent-return-statements,too-few-public-methods,too-many-public-methods,unused-argument,unnecessary-lambda",
    ]
    if session.posargs:
        paths = session.posargs
    else:
